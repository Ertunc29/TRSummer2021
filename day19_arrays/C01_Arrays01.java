package day19_arrays;

import java.util.Arrays;

public class C01_Arrays01 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

		short arr1 [] = new short [5];
		System.out.println(arr1); // arr1 bir array. data turu non-primitive
		                          // short array'in degil icine koyacagimiz degerlerin data turu
		 // eger non-primitive bir datayi direk syso ile yazdirmaya calsiirsaniz 
		 // java arr1 'in referans bilgisini yazdirir 
		
		// soru 1 : elemanlari "ali", "ayse", veli, fatma olan bir array olusturun
		// ve bu arrayi yazdirin 
		
		// 1. yontem : once olusturup sonra istedigim indexlere deger atiyorum
		
		String isimler[] = new String[4];
		
		isimler[0]="Ali";
		isimler[1]="Ayse";
	    isimler[2]="Veli";
		isimler[3]="Fatma";
		
		// 2. yontem : hem olusturup hemde tum indexlere deger atiyorum 
		
		String isimler2[]= {"Ali", "Veli", "Ayse", "Fatma"};
		
		// isimler array'indeki veli yerine hasan yazalim
		
		isimler[1]="Hasan";
		//isimler[5]="Hakan"; // java index var mi yok mu bilmiyor
		                    // syntax acisindan sorun olmadigi icin CTE vermiyor
		                    // ama run ettigimizde 5. index olmadigindan RTE verir
		
		System.out.println(isimler.length);//4 
		
		System.out.println(isimler[3]); // Fatma
		
		System.out.println(isimler[1]);// Hasan
		
		
		// tum elemanlari yazdirma
		//1. yol loop kullanma 
		// sadece degerleri yazdirdik
		
		for (int i=0; i<isimler2.length ; i++) {
			System.out.print(isimler2[i]+ " ");
		}
		
		//2.yol Arrays class'indan yardim aliriz
		// eger array'i direk yazdirmak istersek 
		// Arrays.totring() methodunu kullanmali
		// ve parametre olarak array'in ismini yazmaliyiz
		
		System.out.println("");
		System.out.println(Arrays.toString(isimler));
	}

}
